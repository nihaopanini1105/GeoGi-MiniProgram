...
class MiniProgramBridgeClient:
    ...
    def miniprogram_users(self) -> dict[str, Any]:
        data = self._request("GET", "/internal/os/miniprogram-users")
        users = data.get("users") or []
        channels = data.get("channels") or []
        source_options = data.get("sourceOptions") or []
        if not all(isinstance(value, list) for value in (users, channels, source_options)):
            raise MiniProgramBridgeError("miniprogram_bridge_invalid_user_dashboard")
        return {
            "users": [dict(item) for item in users if isinstance(item, dict)],
            "channels": [dict(item) for item in channels if isinstance(item, dict)],
            "sourceOptions": [dict(item) for item in source_options if isinstance(item, dict)],
        }

    def update_miniprogram_user(self, user_id: str, payload: dict[str, Any]) -> dict[str, Any]:
        clean_user_id = str(user_id or "").strip()
        if not clean_user_id:
            raise MiniProgramBridgeError("miniprogram_user_id_required")
        data = self._request(
            "POST",
            f"/internal/os/miniprogram-users/{quote(clean_user_id, safe='')}",
            dict(payload),
        )
        user = data.get("user")
        if not isinstance(user, dict):
            raise MiniProgramBridgeError("miniprogram_bridge_invalid_user_response")
        return dict(user)
    ...
    def channel_dashboard(self) -> dict[str, Any]:
        ...
        user_options = data.get("userOptions") or []
        if not all(isinstance(value, list) for value in (channels, sources, orders, commissions, monthly, owner_stats, user_options)):
        ...
        return {
            ...
            "userOptions": [dict(item) for item in user_options if isinstance(item, dict)],
        }
