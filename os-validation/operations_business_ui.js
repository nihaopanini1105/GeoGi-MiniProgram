(function(){
  ...
  window.openMiniProgramUserEditor=function(userId){
    ...
    const row=(window._miniProgramUsers||[]).find(x=>String(x.userId)===String(userId));if(!row)return alert('没有找到该小程序用户');
    const selected=new Set((row.channels||[]).map(x=>String(x.channelId||'')));
    const channelOptions=(window._miniProgramUserChannels||[]).map(x=>'<option value="'+esc(x.channelId)+'" '+(selected.has(String(x.channelId||''))?'selected':'')+'>'+esc(x.name||'未命名渠道')+(x.active===false?'（已停用）':'')+'</option>').join('');
    modal('设置小程序用户',
      '<form id="miniProgramUserEditorForm" class="form-grid" autocomplete="off">'+
        ...
        '<div class="field full"><label>设置渠道</label><select name="channel_ids" multiple size="'+Math.min(8,Math.max(3,(window._miniProgramUserChannels||[]).length))+'">'+channelOptions+'</select><small>按住 Command/Ctrl 可多选。这里只建立该用户与渠道的负责人关系，不会自动修改来源归属。</small></div>'+
    ...
    document.getElementById('miniProgramUserEditorForm').onsubmit=async function(e){
      ...
      const channelIds=[...e.target.querySelector('[name="channel_ids"]').selectedOptions].map(x=>x.value).filter(Boolean);
  ...
  window.openChannelEditor=function(channelId=''){
    ...
    const selectedOwners=new Set((row.ownerPhones||[]).map(String));
    const users=window._miniProgramUserOptions||[];
    const ownerOptions=users.map(x=>'<option value="'+esc(x.phoneNumber||'')+'" '+(selectedOwners.has(String(x.phoneNumber||''))?'selected':'')+'>'+esc(verifiedUserOptionLabel(x))+'</option>').join('');
    const ownerControl=users.length
      ? '<select name="owner_phones" multiple size="'+Math.min(8,Math.max(3,users.length))+'">'+ownerOptions+'</select><small>仅可选择已完成微信手机号授权的小程序用户；按住 Command/Ctrl 可多选。</small>'
      : '<div class="notice warn">当前还没有可选择的已验证小程序用户。请让负责人先在小程序完成手机号授权。</div>';
    modal(channelId?'编辑渠道':'新增渠道',
      ...
      '<div class="field full"><label>渠道负责人</label>'+ownerControl+'</div>'+
    ...
    document.getElementById('channelEditorForm').onsubmit=async function(e){
      ...
      const ownerSelect=e.target.querySelector('[name="owner_phones"]');
      const ownerPhones=ownerSelect?[...ownerSelect.selectedOptions].map(x=>x.value).filter(Boolean):[];
  ...
  window.openSourceEditor=function(sourceId=''){
    ...
    const ownerOptions='<option value="">不绑定归属人</option>'+users.map(x=>'<option value="'+esc(x.phoneNumber||'')+'" '+(String(row.ownerPhone||'')===String(x.phoneNumber||'')?'selected':'')+'>'+esc(verifiedUserOptionLabel(x))+'</option>').join('');
    modal(sourceId?'编辑来源入口':'新增来源入口',
      ...
      '<div class="field full"><label>来源归属人</label><select name="owner_phone">'+ownerOptions+'</select><small>只显示已完成微信手机号授权的小程序用户。归属人用于人员统计，不等同于渠道。</small></div>'+
    ...
    document.getElementById('sourceEditorForm').onsubmit=async function(e){
      e.preventDefault();
      const fd=new FormData(e.target),sourceType=String(fd.get('source_type')||''),channelId=String(fd.get('channel_id')||''),ownerPhone=String(fd.get('owner_phone')||'');
      if(sourceType==='channel'&&!channelId)return alert('“渠道分享”类型必须绑定一个渠道');
      const user=(window._miniProgramUserOptions||[]).find(x=>String(x.phoneNumber||'')===ownerPhone);
