(function(){
  ...
  window.render=async function(){
    ...
    const coreViews=new Set([
      'dashboard','clients','intakes','client',
  ...
  async function loadChannelCenterData(){
    ...
    const channels=data.channels||[],sources=data.sources||[],monthly=data.monthly||[],orders=data.orders||[],commissions=data.commissions||[],ownerStats=data.ownerStats||[];
    ...
    return {data,channels,sources,monthly,orders,commissions,ownerStats,canWrite:window._channelCanWrite,canSettle:window._channelCanSettle};
  }
  ...
  async function channelManagement(){
    setTitle('渠道管理','渠道管理','维护渠道、有效期、客户权益、返佣规则与来源入口。');
    try{
      const {channels,sources,canWrite}=await loadChannelCenterData();
      content.innerHTML=
        '<section class="biz-card"><div class="biz-card-head"><div><h3>合作渠道</h3><p>客户无需输入任何渠道码。来源入口绑定渠道后，系统自动按该渠道规则计算优惠与返佣。</p></div>'+
          (canWrite?'<button class="primary-action" onclick="openChannelEditor()">新增渠道</button>':'')+
        '</div><div class="biz-card-body"><div class="table-scroll"><table class="business-table"><thead><tr><th>渠道</th><th>客户权益</th><th>返佣</th><th>有效期</th><th>负责人手机号</th><th>状态</th><th>操作</th></tr></thead><tbody>'+
        (channels.map(row=>'<tr><td><strong>'+esc(row.name||'未命名渠道')+'</strong></td><td>'+esc(channelDiscountLabel(row))+'</td><td>'+esc(channelCommissionLabel(row))+'</td><td><small>'+esc(formatDate(row.startsAt))+'<br>至 '+esc(formatDate(row.endsAt))+'</small></td><td><small>'+esc((row.ownerPhones||[]).join('、')||'未绑定')+'</small></td><td>'+esc(row.active?'生效中':'已停用')+'</td><td><div class="action-row"><button class="secondary-action compact" onclick="openChannelDetail(\''+esc(row.channelId||'')+'\')">明细</button>'+(canWrite?'<button class="secondary-action compact" onclick="openChannelEditor(\''+esc(row.channelId||'')+'\')">编辑</button>':'')+'</div></td></tr>').join('')||'<tr><td colspan="7">当前还没有渠道。点击“新增渠道”设置客户权益与返佣规则。</td></tr>')+
        '</tbody></table></div></div></section><div style="height:14px"></div>'+
        '<section class="biz-card"><div class="biz-card-head"><div><h3>来源入口</h3><p>官网、公众号、名片和渠道专属入口用于来源统计与归因；绑定渠道后自动应用该渠道客户权益。</p></div>'+
          (canWrite?'<button class="primary-action" onclick="openSourceEditor()">新增来源入口</button>':'')+
        '</div><div class="biz-card-body"><div class="table-scroll"><table class="business-table"><thead><tr><th>来源</th><th>类型</th><th>归属人</th><th>绑定渠道</th><th>状态</th><th>访问 / 提交 / 成交</th><th>小程序入口</th><th>操作</th></tr></thead><tbody>'+
        (sources.map(row=>'<tr><td><strong>'+esc(row.name||'未命名来源')+'</strong><br><small>'+esc(row.sourceId||'—')+'</small></td><td>'+esc(row.sourceTypeLabel||sourceTypeLabel(row.sourceType))+'</td><td>'+esc(row.ownerName||'未绑定')+(row.ownerPhone?'<br><small>'+esc(row.ownerPhone)+'</small>':'')+'</td><td>'+esc(row.channelName||'不绑定渠道')+'</td><td>'+esc(sourceStatusLabel(row))+'</td><td>'+esc(row.visits||0)+' / '+esc(row.submittedOrders||0)+' / '+esc(row.paidOrders||0)+'</td><td><small>'+esc(row.miniProgramPath||'—')+'</small><br>'+(row.codeUrl?'<a href="'+esc(row.codeUrl)+'" target="_blank" rel="noopener">查看小程序码</a>':'<small>尚未生成小程序码</small>')+'</td><td><div class="action-row">'+(canWrite?'<button class="secondary-action compact" onclick="openSourceEditor(\''+esc(row.sourceId||'')+'\')">编辑</button><button class="secondary-action compact" onclick="generateSourceCode(\''+esc(row.sourceId||'')+'\')">'+(row.codeUrl?'重新生成码':'生成小程序码')+'</button>':'—')+'</div></td></tr>').join('')||'<tr><td colspan="8">暂无来源入口。</td></tr>')+
        '</tbody></table></div></div></section>';
  ...
  window.openChannelEditor=function(channelId=''){
    ...
    modal(channelId?'编辑渠道':'新增渠道','<form id="channelEditorForm" class="form-grid" autocomplete="off"><input type="hidden" name="channel_id" value="'+esc(row.channelId||'')+'"><div class="field"><label>渠道名称</label><input name="name" autocomplete="off" value="'+esc(row.name||'')+'" placeholder="例如：A 渠道" required></div><div class="field"><label>客户优惠类型</label><select name="discount_type"><option value="percent" '+(row.discountType!=='free'?'selected':'')+'>折扣</option><option value="free" '+(row.discountType==='free'?'selected':'')+'>本次诊断免费</option></select></div><div class="field"><label>客户折扣（折）</label><input name="discount_fold" type="number" min="0.1" max="10" step="0.1" value="'+esc(fold)+'"><small>例如 8 表示客户支付原价的 8 折；选择免费时此值忽略。</small></div><div class="field"><label>返佣比例（%）</label><input name="commission_percent" type="number" min="0" max="100" step="0.01" value="'+esc(commissionPct)+'"></div><div class="field"><label>状态</label><select name="active"><option value="true" '+(row.active!==false?'selected':'')+'>生效中</option><option value="false" '+(row.active===false?'selected':'')+'>停用</option></select></div><div class="field"><label>生效时间</label><input name="starts_at" type="datetime-local" value="'+esc(channelLocalInput(row.startsAt))+'"></div><div class="field"><label>失效时间</label><input name="ends_at" type="datetime-local" value="'+esc(channelLocalInput(row.endsAt))+'"></div><div class="field full"><label>渠道负责人手机号</label><textarea name="owner_phones" rows="2" placeholder="可填写多个，用逗号分隔">'+esc((row.ownerPhones||[]).join(','))+'</textarea><small>绑定手机号授权登录小程序后，会在“我的”看到该渠道自己的推广和返佣数据。</small></div><div class="field full"><label>备注</label><textarea name="notes" rows="3" placeholder="渠道说明、合作约定等">'+esc(row.notes||'')+'</textarea></div><div class="notice">有效期和规则可以修改；修改只影响之后创建的新订单，历史订单仍按成交时快照计算。</div><div class="form-actions"><button type="button" class="secondary" onclick="closeModal()">取消</button><button class="primary">保存渠道</button></div></form>');
    document.getElementById('channelEditorForm').onsubmit=async function(e){
      e.preventDefault();const fd=new FormData(e.target),discountType=String(fd.get('discount_type')||'percent'),foldValue=Number(fd.get('discount_fold')||0),commissionPercent=Number(fd.get('commission_percent')||0);
      ...
      const payload={channelId:String(fd.get('channel_id')||''),name:String(fd.get('name')||'').trim(),discountType,discountRateBps:discountType==='free'?0:Math.round(foldValue*1000),commissionRateBps:Math.round(commissionPercent*100),active:String(fd.get('active'))==='true',startsAt:toIso(fd.get('starts_at')),endsAt:toIso(fd.get('ends_at')),ownerPhones:String(fd.get('owner_phones')||'').split(/[、,，;；\\s]+/).map(x=>x.trim()).filter(Boolean),notes:String(fd.get('notes')||'').trim()};
  ...
  window.openSourceEditor=function(sourceId=''){
    ...
    modal(sourceId?'编辑来源入口':'新增来源入口','<form id="sourceEditorForm" class="form-grid" autocomplete="off"><input type="hidden" name="source_id" value="'+esc(row.sourceId||'')+'"><div class="field"><label>来源名称</label><input name="name" autocomplete="off" value="'+esc(row.name||'')+'" placeholder="例如：GeoGi 官网首页 / A渠道商务名片" required></div><div class="field"><label>来源类型</label><select name="source_type"><option value="website" '+(row.sourceType==='website'?'selected':'')+'>官网</option><option value="official_account" '+(row.sourceType==='official_account'?'selected':'')+'>公众号</option><option value="business_card" '+(row.sourceType==='business_card'?'selected':'')+'>名片</option><option value="channel" '+(row.sourceType==='channel'?'selected':'')+'>渠道分享</option></select></div><div class="field"><label>归属人</label><input name="owner_name" autocomplete="off" value="'+esc(row.ownerName||'')+'" placeholder="例如：李沙沙"></div><div class="field"><label>归属手机号</label><input name="owner_phone" type="tel" inputmode="numeric" autocomplete="off" value="'+esc(row.ownerPhone||'')+'" placeholder="绑定该人员登录小程序的手机号"><small>用于人员归属统计与小程序账号识别，不等同于渠道负责人。</small></div><div class="field full"><label>绑定渠道</label><select name="channel_id">'+channelOptions+'</select><small>用于来源归因和渠道统计；绑定渠道后自动应用该渠道客户权益与返佣规则。</small></div><div class="field"><label>状态</label><select name="active"><option value="true" '+(row.active!==false?'selected':'')+'>生效中</option><option value="false" '+(row.active===false?'selected':'')+'>停用</option></select></div><div class="field"><label>生效时间</label><input name="starts_at" type="datetime-local" value="'+esc(channelLocalInput(row.startsAt))+'"></div><div class="field"><label>失效时间</label><input name="ends_at" type="datetime-local" value="'+esc(channelLocalInput(row.endsAt))+'"></div><div class="field full"><label>备注</label><textarea name="notes" rows="3" placeholder="例如：官网首页、2026版商务名片、渠道线下活动等">'+esc(row.notes||'')+'</textarea></div><div class="form-actions"><button type="button" class="secondary" onclick="closeModal()">取消</button><button class="primary">保存来源</button></div></form>');
    document.getElementById('sourceEditorForm').onsubmit=async function(e){
      e.preventDefault();const fd=new FormData(e.target),sourceType=String(fd.get('source_type')||''),channelId=String(fd.get('channel_id')||'');
      if(sourceType==='channel'&&!channelId)return alert('“渠道分享”类型必须绑定一个渠道');
      const toIso=v=>{if(!v)return '';const d=new Date(String(v));return Number.isNaN(d.getTime())?'':d.toISOString()};
      const payload={sourceId:String(fd.get('source_id')||''),name:String(fd.get('name')||'').trim(),sourceType,ownerName:String(fd.get('owner_name')||'').trim(),ownerPhone:String(fd.get('owner_phone')||'').trim(),channelId,active:String(fd.get('active'))==='true',startsAt:toIso(fd.get('starts_at')),endsAt:toIso(fd.get('ends_at')),notes:String(fd.get('notes')||'').trim()};
  ...
  async function businessClients(){
    ...
    const rows=items.map(row=>{
      ...
      return '<tr class="submitted-customer-row" data-search="'+esc(submittedCustomerSearchText(row))+'">'+
        ...
        '<td>'+esc(row.contactName||'—')+'<br><small>'+esc(row.contactMethod||'联系方式待同步')+'</small></td>'+
