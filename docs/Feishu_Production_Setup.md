...
1. 小程序请求你的 HTTPS 服务器接口 `POST /api/diagnosis/submit`
2. 服务器校验字段并生成 `client_id` / `project_id`
3. 服务器通过飞书开放平台写入多维表格
4. 服务器发送飞书通知给负责人
5. 小程序展示提交成功页和客户编号
...
1. 运营在飞书内容表维护文章
2. 小程序请求你的 HTTPS 服务器接口 `GET /api/research/articles`
...
| 字段名 | 类型 | 是否必填 | 说明 |
| --- | --- | --- | --- |
| 客户编号 | 文本 | 是 | 服务器生成，例如 `GG-202607-123456` |
...
说明 `GET /api/research/articles` 未读取到飞书文章。
