...
async function createBitableRecords({ tenantToken, appToken, tableId, records }) {
  if (!records.length) {
    return { records: [] };
  }
  ...
  const result = await requestJson({
    method: 'POST',
    hostname: 'open.feishu.cn',
    path: `/open-apis/bitable/v1/apps/${appToken}/tables/${tableId}/records/batch_create?client_token=${crypto.randomUUID()}`,
    headers: {
      Authorization: `Bearer ${tenantToken}`
    },
    body: {
  ...
  if (result.code !== 0) {
    throw new Error(`batch create records failed: ${JSON.stringify(result)}`);
  }
  return result.data;
}
...
async function deleteBitableRecord({ tenantToken, appToken, tableId, recordId }) {
  const result = await requestJson({
    method: 'DELETE',
    hostname: 'open.feishu.cn',
    path: `/open-apis/bitable/v1/apps/${appToken}/tables/${tableId}/records/${recordId}`,
  ...
  if (result.code !== 0) {
    throw new Error(`delete record failed: ${JSON.stringify(result)}`);
  }
  return result.data;
}
